...
holdout_predictions <- holdout |>
  ...
  mutate(
    model_prediction = estimate_1rm(
      ...
      summarise(prediction = mean(est_1RM)) |> # could do median?
      pull(prediction),
...
```{r}
athletes_sample_varying_obs <- train |>
...
plot2_data <- athletes_sample_varying_obs |>
  mutate(
    estimate = map(
      athlete,
      \(x) estimate_1rm(modified_epley_model_fit, x, 100, 8)
    ),
    athlete = factor(athlete, levels = athletes_sample_varying_obs$athlete)
  ) |>
