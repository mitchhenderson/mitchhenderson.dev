...
The model will also be a [*Bayesian*](https://en.wikipedia.org/wiki/Bayesian_statistics) model. These have advantages over [conventional](https://en.wikipedia.org/wiki/Frequentist_probability) statistical models when it comes to expressing uncertainty ([among other things](https://www.youtube.com/watch?v=R6d-AbkhBQ8)). So instead of just getting a number, you get a full distribution representing the uncertainty in the 1RM estimate (e.g., "The 1RM estimate is 100`r units` with 80% probability of it being between 98 and 102`r units`").[^1] This distribution accounts for all the sources of upstream uncertainty (amount of data available on the athlete lifting, number of reps they completed, etc).
...
::: {.details summary="**Optional** technical simulation details"}
...
::: {.details summary="Optional stats details about the model"}
...
example_predictions |>
  ...
  scale_x_continuous(breaks = seq(116, 122, 1), labels = label_number(suffix = units)) +
...
By seeing how close the model got to the true 1RM (from the simulation), and doing the same for the standard Epley formula, we can compare their predictive accuracy for new data.[^3]
...
holdout_predictions <- holdout |>
  ...
  mutate(
    model_prediction = estimate_1rm(
      ...
      summarise(prediction = median(est_1RM)) |> # could do median?
      pull(prediction),
  ...
  mutate(
    ...
    most_accurate_method = case_when(
      ...
      difference < 0 ~ paste0("Model closer by ", round(abs(difference), 1), units),
      difference > 0 ~ paste0("Formula closer by ", round(abs(difference), 1), units),
    )
  )
...
endurance_profiles |>
  ...
  scale_colour_manual(values = c("#16A34A", "#2563EB", "#DB2777")) +
  scale_fill_manual(values = c("#16A34A", "#2563EB", "#DB2777")) +
  labs(
    x = "Repetitions",
...
To demonstrate, here's 4 athletes' estimates after hypothetically performing a max reps set of 8 @ 100`r units`, each with a different amount of historical data used to train the model.
...
Multi model or model averaging approach might be the best.

My simulated data might not align with the patterns in real athletes, but I think the idea that some can do heavy weight well while others do higher reps well is valid and should/could be captured in 1RM estimates.

Integrating this into a larger system would involve deploying the model on a server (probably on cloud but could also be on a physical machine you or your team own). Predictions could then be generated by making requests to the model's API using your analysis tool of choice (R, Python, Excel via Power Query, etc).
