...
To work it out, I scraped 10 seasons of match result data from Wikipedia (excluding the shortened 2020 season) and saved the results in a file.
...
All 10 seasons can then be visualised together as a dot plot. Notice how one team won 10 games and made the finals (blue circle at the top of the 10 wins column) and another team won 13 games but didn't qualify (red circle at the bottom of the 13 wins column).
...
We can use the function we defined earlier to summarise the match level simulation results into an end-of-season ladder for each of the 10,000 seasons. Here is one of those seasons final standings (for season number 2534 out of 10,000).
...
probabilities_simulated |>
  ...
  cols_label_with(columns = list_data, 
                  fn = function(label) {
                    full_label <- glue::glue("Finals probabilities<br>{label}")
...
In summary, Monte Carlo simulations offer a practical way to estimate things (like finals probabilities in the NRL), especially when the calculations become mathematically complex or the sample of available data is relatively small.
