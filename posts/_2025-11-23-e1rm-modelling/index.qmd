...
holdout_predictions <- holdout |>
  ...
  mutate(
    ...
    most_accurate_method = case_when(
      ...
      difference < 0 ~ paste("Model closer by", round(abs(difference), 1)),
      difference > 0 ~ paste("Formula closer by", round(abs(difference), 1)),
    )
  )
...
Mean absolute error for the model was **`r round(mean(abs(holdout_predictions$model_error)), 1)`**.

Mean absolute error for the regular Epley formula was **`r round(mean(abs(holdout_predictions$standard_epley_error)), 1)`**.

This means that the model will be **`r round(mean(abs(holdout_predictions$standard_epley_error)) - mean(abs(holdout_predictions$model_error)), 1)` more accurate on average**.[^4]

[^4]: Take with grain of salt as it's only an average over 30 estimates.

## What has the model learned about the athletes?

To demonstrate this point, I'll just show three athletes: the highest endurance (Athlete 12), the lowest endurance (Athlete 24), and one in the middle (Athlete 17).
...
ggplot(
  ...
  geom_textline(aes(label = label), hjust = 0.75, family = "Myriad Pro Regular") +
...
plot2_data <- holdout_subset |>
  pmap_dfr(\(
    ...
    estimate_1rm(modified_epley_model_fit, athlete, weight, reps) |>
      mutate(
        true_1rm = true_1rm,
...
ggplot(plot2_data, aes(x = est_1RM, y = reorder(label, n_obs))) +
