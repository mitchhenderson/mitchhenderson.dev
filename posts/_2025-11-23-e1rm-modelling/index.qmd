...
example_predictions |>
  ...
  scale_x_continuous(breaks = seq(116, 122, 1), labels = scales::label_number(suffix = units)) +
...
## How much does having training data impact estimate certainty?

Text.

```{r}
athletes_varying_obs <- train |>
  count(athlete, name = "n_obs", sort = T) |>
  slice(c(1, 5, 10, 15, 20, 25, 30))

# more data doesn't always mean more certainty about the prediction — sometimes it means more certainty that the athlete is inherently variable.
...
data_varying_obs <- athletes_varying_obs |>
  mutate(
    ...
    athlete = factor(
      athlete,
      levels = athletes_varying_obs$athlete,
      labels = paste0(
        "Athlete ",
        athletes_varying_obs$athlete,
        " (",
        n_obs,
...
# Make it taller
# hline after stat_slab
# Make geom_hline grey (same as x axis)
# maybe annotate?
# number of historical data should be grey and maybe smaller?
data_varying_obs |>
  ggplot(aes(x = est_1RM)) +
  ...
  theme(
    ...
    strip.text = element_text(family = "Myriad Pro Regular", hjust = 0)
  )
```
