...
There are a few ways to go about calculating a probability for making the finals based on a given number of wins. To do this requires a shift in mindset from thinking only about the outcome of a single event (qualify vs not) to considering the expected frequency of an outcome occurring over a large number of events involving uncertainty and randomness.
